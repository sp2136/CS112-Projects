...
public class Board {
    ...
    public void addRandomTile() {
        ...
        int row = openSpaces.get(tile).getRow();
        int col = openSpaces.get(tile).getCol();
    ...
    public void swipeLeft() {
        ...
        for(int i = 0; i < gameBoard.length; i++)
        {
            ...
            for(int j = 0; j < gameBoard[0].length; j++)
            {
                if(gameBoard[i][j] != 0)
                {
                    gameBoard[i][0 + plus] = gameBoard[i][j];
    ...
    public void transpose() {
        ...
        int[][] g = new int[gameBoard.length][gameBoard[0].length];
        for(int i = 0; i < gameBoard.length; i++)
        {
            for(int j = 0; j < gameBoard[0].length; j++)
            {
    ...
    public void flipRows() {
        ...
        {
            int first = 0;
            int last = gameBoard[0].length - 1;

            while(first < last)
            {
                int temp = gameBoard[i][first];
                gameBoard[i][first] = gameBoard[i][last];
                gameBoard[i][last] = temp;
